...
export default function Inputs(props){
    ...
    const validationHour = (value)=>{
        ...
        if (verifyHour && verifyHour.length === value.length) {
            setViewComponent([false,false])
            putInList(dataList.name,dataList.date,{hour:value,mins:(dataList.time.mins==="" ? "00":"")}, dataList.description)
    ...
    const sendAndClearFields=()=>{
        ...
        if(name==="" || date===""){
            console.log("Erro no name/date")
            setValues([values[0],values[1],values[2],values[3],"ERRO: Name or date is empty"])
        }
        else if(time.hour==="" && time.mins.length>1){
            console.log("Erro no time")
            setValues([values[0],values[1],values[2],values[3],"ERRO: U put the minute, but not the hour"])
        }
